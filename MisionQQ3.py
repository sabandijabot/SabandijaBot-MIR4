...
# COLORES PARA BOTONES DESACTIVADOS
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("SABANDIJA B0T - REPTIL EDITION v13 (PERFILES)")
        self.root.geometry("720x1050")
        ...
        self.log_status("🛡️ Sistema de Perfiles activado. Crea, guarda y ejecuta tus perfiles.")
    ...
    # --- LÓGICA DE PERFILES ---
    def guardar_configuracion_perfil(self):
        nombre_perfil = self.entry_apodo.get().strip()
        if not nombre_perfil:
            messagebox.showwarning("Atención", "Debes ingresar un APODO para guardar el perfil.")
            return False

        if not self.config.has_section(nombre_perfil):
            self.config.add_section(nombre_perfil)

        self.config.set(nombre_perfil, "modo", self.modo_var.get())
        self.config.set(nombre_perfil, "delay", self.entry_delay.get())
        self.config.set(nombre_perfil, "ulti", str(self.ulti_var.get()))
        
        sel = self.combo_ventanas.get()
        titulo_limpio = sel.split(" (HWND:")[0].strip() if sel else ""
        self.config.set(nombre_perfil, "ventana_titulo", titulo_limpio)
        
        self.guardar_config()
        self.log_status(f"💾 Perfil [{nombre_perfil}] guardado exitosamente.")
        self.actualizar_tabla_visual()
        return True

    def eliminar_perfil_seleccionado(self):
        if not self.tabla_instancias.selection():
            messagebox.showwarning("Atención", "Selecciona un perfil de la tabla para eliminar.")
            return
        
        valores = self.tabla_instancias.item(self.tabla_instancias.selection()[0], "values")
        apodo = str(valores[1])
        
        hwnd_val = str(valores[0])
        if hwnd_val.isdigit() and int(hwnd_val) in self.instancias_activas:
            messagebox.showwarning("Atención", "No puedes eliminar un perfil que está en ejecución. Detenlo primero.")
            return
            
        if self.config.has_section(apodo):
            if messagebox.askyesno("Confirmar", f"¿Seguro que quieres eliminar el perfil [{apodo}]?"):
                self.config.remove_section(apodo)
                self.guardar_config()
                self.log_status(f"🗑️ Perfil [{apodo}] eliminado.")
                self.actualizar_tabla_visual()
                self.entry_apodo.delete(0, tk.END)
        else:
            messagebox.showwarning("Atención", "Ese no es un perfil guardado válido.")

    def cargar_configuracion_perfil(self, nombre_perfil):
        if not nombre_perfil: return
        
        if self.config.has_section(nombre_perfil):
            self.modo_var.set(self.config.get(nombre_perfil, "modo", fallback="MISION_Q"))
            self.actualizar_radio_colores()
            self.entry_delay.delete(0, tk.END)
            self.entry_delay.insert(0, self.config.get(nombre_perfil, "delay", fallback="5"))
            self.ulti_var.set(self.config.getboolean(nombre_perfil, "ulti", fallback=False))
            
            ventana_guardada = self.config.get(nombre_perfil, "ventana_titulo", fallback="")
            if ventana_guardada:
            ...
            self.log_status(f"⚙️ Perfil [{nombre_perfil}] cargado en interfaz.")
        else:
            self.log_status(f"⚠️ Perfil [{nombre_perfil}] no existe en config.ini.")
    ...
    def setup_tab_bot(self):
        ...
        self.combo_ventanas.bind("<<ComboboxSelected>>", lambda e: self.evaluar_estado_botones_seleccion())
        tk.Button(frame_sel, text="🔄 REFRESCAR", bg=COLOR_CARD, fg=COLOR_ACCENT, relief="groove", font=('Segoe UI', 9, 'bold'), activebackground=COLOR_ACCENT, command=self.actualizar_combo).pack(pady=10, padx=10, side="right")
        ...
        self.rb_mision = tk.Radiobutton(frame_cfg, text="MISIÓN Q (AUTO)", variable=self.modo_var, value="MISION_Q", bg=COLOR_BG, fg=COLOR_TEXT_ON, selectcolor="#000", font=('Segoe UI', 10, 'bold'), command=on_mode_change)
        self.rb_mision.pack(anchor="w", padx=20, pady=2)
        self.rb_farma = tk.Radiobutton(frame_cfg, text="FARMA EXP (AFK)", variable=self.modo_var, value="FARMA_EXP", bg=COLOR_BG, fg=COLOR_TEXT_OFF, selectcolor="#000", font=('Segoe UI', 10, 'bold'), command=on_mode_change)
        self.rb_farma.pack(anchor="w", padx=20, pady=2)
        self.rb_summon = tk.Radiobutton(frame_cfg, text="SUMMON BOSS (TECLA E)", variable=self.modo_var, value="SUMMON_BOSS", bg=COLOR_BG, fg=COLOR_TEXT_OFF, selectcolor="#000", font=('Segoe UI', 10, 'bold'), command=on_mode_change)
        self.rb_summon.pack(anchor="w", padx=20, pady=2)
        
        self.check_ulti = tk.Checkbutton(frame_cfg, text="LANZAR ULTI [R] (Solo Farma)", variable=self.ulti_var, bg=COLOR_BG, fg=COLOR_TEXT_OFF, selectcolor="#000", state="disabled", activebackground=COLOR_BG, activeforeground=COLOR_SECONDARY, font=('Segoe UI', 9, 'bold'), command=self.evaluar_estado_botones_seleccion)
        ...
        tk.Label(inputs_frame, text="DELAY (SEG):", bg=COLOR_BG, fg=COLOR_ACCENT, font=('Segoe UI', 9, 'bold')).pack(side="left")
        ...
        self.entry_delay.bind("<KeyRelease>", lambda e: self.evaluar_estado_botones_seleccion())
        
        tk.Label(inputs_frame, text="APODO (PERFIL):", bg=COLOR_BG, fg=COLOR_ACCENT, font=('Segoe UI', 9, 'bold')).pack(side="left", padx=(15, 0))
        ...
        self.entry_apodo.bind("<FocusOut>", lambda e: self.cargar_configuracion_perfil(self.entry_apodo.get().strip()))
        self.entry_apodo.bind("<KeyRelease>", lambda e: self.evaluar_estado_botones_seleccion())

        # --- TABLA DE INSTANCIAS Y PERFILES ---
        frame_grid_instancias = ttk.LabelFrame(self.tab_bot, text=" INSTANCIAS Y PERFILES GUARDADOS ", style="Card.TLabelframe")
        frame_grid_instancias.pack(fill="x", padx=15, pady=5)
        self.tabla_instancias = ttk.Treeview(frame_grid_instancias, columns=("hwnd", "apodo", "modo", "delay", "estado"), show="headings", height=8)
        for col, txt, w in [("hwnd","Identificador",90), ("apodo","Apodo",130), ("modo","Modo",130), ("delay","Delay",70), ("estado","Estado",110)]:
        ...
        # --- NUEVA FILA DE BOTONES: GESTIÓN DE PERFILES ---
        btn_perfil_frame = tk.Frame(self.tab_bot, bg=COLOR_BG); btn_perfil_frame.pack(pady=2, fill="x", padx=15)
        tk.Button(btn_perfil_frame, text="💾 GUARDAR PERFIL", bg=COLOR_CARD, fg=COLOR_SECONDARY, font=('Segoe UI', 9, 'bold'), relief="groove", command=self.guardar_configuracion_perfil).pack(side="left", fill="x", expand=True, padx=2)
        tk.Button(btn_perfil_frame, text="🗑️ ELIMINAR PERFIL", bg="#330000", fg="#ff0000", font=('Segoe UI', 9, 'bold'), relief="groove", command=self.eliminar_perfil_seleccionado).pack(side="left", fill="x", expand=True, padx=2)

        # --- BOTONES DE CONTROL DE EJECUCIÓN ---
        btn_frame = tk.Frame(self.tab_bot, bg=COLOR_BG); btn_frame.pack(pady=5, fill="x", padx=15)
        
        self.btn_desplegar = tk.Button(btn_frame, text="🚀 DESPLEGAR", font=('Segoe UI', 9, 'bold'), relief="flat", command=self.activar_instancia)
        self.btn_desplegar.pack(side="left", fill="x", expand=True, padx=2)
        
        self.btn_actualizar = tk.Button(btn_frame, text="⚡ ACTUALIZAR", font=('Segoe UI', 9, 'bold'), relief="flat", command=self.actualizar_instancia_en_caliente)
        self.btn_actualizar.pack(side="left", fill="x", expand=True, padx=2)
        
        self.btn_pausar = tk.Button(btn_frame, text="⏸️ PAUSAR", font=('Segoe UI', 9, 'bold'), relief="flat", command=self.pausar_instancia)
        self.btn_pausar.pack(side="left", fill="x", expand=True, padx=2)
        
        self.btn_reanudar = tk.Button(btn_frame, text="▶️ REANUDAR", font=('Segoe UI', 9, 'bold'), relief="flat", command=self.reanudar_instancia)
        self.btn_reanudar.pack(side="left", fill="x", expand=True, padx=2)
        
        self.btn_detener = tk.Button(btn_frame, text="🛑 DETENER", font=('Segoe UI', 9, 'bold'), relief="flat", command=self.detener_instancia)
        ...
        self.log_status(f"SabandijaBot v13 (PERFILES) inicializado. [{estado_uia}]")

    # --- FUNCIÓN MEJORADA: Cambiar colores de los botones ---
    def evaluar_estado_botones_seleccion(self, event=None):
        seleccion = self.tabla_instancias.selection()
        
        if not seleccion:
            self.btn_desplegar.config(state="normal", bg=COLOR_ACCENT, fg="#000")
            self.btn_actualizar.config(state="disabled", bg=COLOR_DISABLED_BG, fg=COLOR_DISABLED_FG)
        ...
        hwnd_val = str(valores[0])
        apodo_val = str(valores[1])
        
        hay_cambios = False
        try:
            delay_actual = float(self.entry_delay.get())
        except ValueError:
            delay_actual = -1.0
            
        ulti_actual = self.ulti_var.get()
        modo_actual = self.modo_var.get()
        apodo_actual = self.entry_apodo.get().strip()

        if hwnd_val.isdigit() and int(hwnd_val) in self.instancias_activas:
            bot = self.instancias_activas[int(hwnd_val)]
            
            self.btn_desplegar.config(state="disabled", bg=COLOR_DISABLED_BG, fg=COLOR_DISABLED_FG)
            ...
            if bot.modo != modo_actual: hay_cambios = True
            elif bot.delay_personalizado != delay_actual: hay_cambios = True
            elif bot.usar_ulti != ulti_actual: hay_cambios = True
            elif apodo_actual != bot.apodo: hay_cambios = True
            ...
            if self.config.has_section(apodo_val):
                saved_modo = self.config.get(apodo_val, "modo", fallback="MISION_Q")
                saved_delay_str = self.config.get(apodo_val, "delay", fallback="5")
                saved_ulti = self.config.getboolean(apodo_val, "ulti", fallback=False)
                
                try:
                    saved_delay = float(saved_delay_str)
                except ValueError:
                    saved_delay = -1.0
                    
                if saved_modo != modo_actual: hay_cambios = True
                elif saved_delay != delay_actual: hay_cambios = True
                elif saved_ulti != ulti_actual: hay_cambios = True
                elif apodo_actual != apodo_val: hay_cambios = True
                
        if hay_cambios:
            self.btn_actualizar.config(state="normal", bg=COLOR_SECONDARY, fg="#000")
        else:
            self.btn_actualizar.config(state="disabled", bg=COLOR_DISABLED_BG, fg=COLOR_DISABLED_FG)
    ...
    def cargar_datos_instancia_seleccionada(self, event):
        ...
        if hwnd_val.isdigit() and int(hwnd_val) in self.instancias_activas:
            ...
            self.cargar_configuracion_perfil(apodo_val)
            self.entry_apodo.delete(0, tk.END)
    ...
    def actualizar_tabla_visual(self):
        ...
        hwnd_seleccionado = item_seleccionado_valores[0] if item_seleccionado_valores else None
        apodo_seleccionado = item_seleccionado_valores[1] if item_seleccionado_valores else None
        ...
        for hwnd, bot in self.instancias_activas.items():
            ...
            item = self.tabla_instancias.insert("", tk.END, values=(hwnd, bot.apodo, bot.modo, f"{bot.delay_personalizado}s", estado))
            if str(hwnd) == str(hwnd_seleccionado): self.tabla_instancias.selection_set(item)
            apodos_activos.append(bot.apodo)
            
        for section in self.config.sections():
            if section in ['RUTAS', 'LAUNCHER']: continue
            if section not in apodos_activos:
                modo = self.config.get(section, "modo", fallback="MISION_Q")
                delay = self.config.get(section, "delay", fallback="5")
                item = self.tabla_instancias.insert("", tk.END, values=("PERFIL", section, modo, f"{delay}s", "GUARDADO 💾"))
    ...
    def obtener_hwnd_seleccionado(self):
        ...
        sel = self.combo_ventanas.get()
        if sel:
            try: return int(sel.split("(HWND: ")[1].replace(")", ""))
            except: pass
        messagebox.showwarning("Atención", "Selecciona una ventana válida o despliega el perfil primero."); return None

    def activar_instancia(self):
        apodo = self.entry_apodo.get().strip()
        if not apodo:
            messagebox.showwarning("Atención", "Debes ingresar un APODO (PERFIL) para desplegar.")
            return
            
        sel = self.combo_ventanas.get()
        if not sel: messagebox.showwarning("Atención", "No se seleccionó ventana."); return
        try:
            hwnd = int(sel.split("(HWND: ")[1].replace(")", ""))
            if hwnd in self.instancias_activas and self.instancias_activas[hwnd].is_alive(): 
                self.log_status(f"⚠️ HWND {hwnd} ya está controlado."); return
            
            self.guardar_configuracion_perfil()
            ...
            bot.start()
            self.log_status(f"✅ [{bot.apodo}] desplegado. Presiona REANUDAR.")
            self.actualizar_tabla_visual()
    ...
    def actualizar_instancia_en_caliente(self):
        ...
        if hwnd and hwnd in self.instancias_activas:
            try:
                bot = self.instancias_activas[hwnd]; bot.modo = self.modo_var.get()
                bot.delay_personalizado = float(self.entry_delay.get())
                if self.entry_apodo.get().strip(): bot.apodo = self.entry_apodo.get().strip()
                bot.usar_ulti = self.ulti_var.get() if bot.modo == "FARMA_EXP" else False
                
                self.guardar_configuracion_perfil()
                
                self.log_status(f"⚙️ [{bot.apodo}] y su perfil fueron actualizados en caliente.")
                self.actualizar_tabla_visual()
            except Exception as e: self.log_status(f"❌ Error: {str(e)}")
        else:
            self.guardar_configuracion_perfil()
    ...
    def setup_tab_layout(self):
        ...
        self.slider = ttk.Scale(frame_grid, from_=0, to=20, orient='horizontal', variable=self.offset_var, command=lambda v: self.label_offset.config(text=f"Ajuste de Borde (Offset): {int(float(v))}"))
    ...
    def ordenar_grid_ventanas(self):
        ...
        if not v_list: self.log_status("⚠️ No hay ventanas Mir4G."); return
        ...
        try:
            ...
            self.log_status(f"⚡ Grid aplicado a {num} ventanas.")
        except Exception as e: self.log_status(f"❌ Error Layout: {str(e)}")

    # --- TAB DESPLIEGUE ---
    def setup_tab_despliegue(self):
        frame_steam = ttk.LabelFrame(self.tab_despliegue, text=" INFRAESTRUCTURA STEAM ", style="Card.TLabelframe")
        ...
        steam_path = self.config['RUTAS']['steam'] if ('RUTAS' in self.config and 'steam' in self.config['RUTAS']) else r"C:\Program Files (x86)\Steam\steam.exe"
        ...
        frame_ruta = ttk.LabelFrame(self.tab_despliegue, text=" RUTA DEL LAUNCHER MIR4 (GAME 1 & 2) ", style="Card.TLabelframe")
        ...
        frame_coord = ttk.LabelFrame(self.tab_despliegue, text=" COORDENADAS DE BOTONES (X, Y) ", style="Card.TLabelframe")
        ...
        frame_extra = ttk.LabelFrame(self.tab_despliegue, text=" AJUSTES FINOS ", style="Card.TLabelframe")
        ...
        tk.Label(frame_extra, text="💡 Usa CLASS:HwndWrapper[Mir4Launcher.exe]", bg=COLOR_BG, fg=COLOR_TEXT_OFF, font=('Segoe UI', 8, 'italic')).pack(anchor="w", padx=20)
        ...
        frame_btn = tk.Frame(self.tab_despliegue, bg=COLOR_BG); frame_btn.pack(fill="x", padx=15, pady=15)
    ...
    def _hilo_captura(self, entry_x, entry_y, original_bg_x, original_bg_y):
        try:
            ...
            while True:
                ...
                if last_state >= 0 and current_state < 0: screen_x, screen_y = win32api.GetCursorPos(); break
            ...
            if hwnd_launcher:
                ...
                if rel_x >= 0 and rel_y >= 0:
                    self.root.after(0, self._terminar_captura, entry_x, entry_y, str(rel_x), str(rel_y), original_bg_x, original_bg_y, True); return
                else:
                    self.root.after(0, self._terminar_captura, entry_x, entry_y, "0", "0", original_bg_x, original_bg_y, False, "Clic fuera de la ventana."); return
            else:
    ...
    def guardar_config_launcher(self):
        ...
        self.config['LAUNCHER']['g1_x'] = self.ent_g1_x.get()
        self.config['LAUNCHER']['g1_y'] = self.ent_g1_y.get()
        self.config['LAUNCHER']['g2_x'] = self.ent_g2_x.get()
        self.config['LAUNCHER']['g2_y'] = self.ent_g2_y.get()
        self.config['LAUNCHER']['titulo'] = self.ent_titulo_launcher.get()
        self.config['LAUNCHER']['delay_pre_click'] = self.ent_delay_pre_click.get()
        self.config['LAUNCHER']['delay'] = self.ent_delay_launch.get()
        self.guardar_config(); self.log_status("💾 Configuración de Launcher guardada.")

    def cargar_config_launcher(self):
        defaults = {'ruta': r"C:\Ruta\A\Tu\Launcher\Mir4Launcher.exe", 'g1_x': '450', 'g1_y': '520', 'g2_x': '450', 'g2_y': '620', 'titulo': 'CLASS:HwndWrapper[Mir4Launcher.exe]', 'delay_pre_click': '3', 'delay': '15'}
        cfg = self.config['LAUNCHER'] if 'LAUNCHER' in self.config else defaults
        self.ent_launcher.insert(0, cfg.get('ruta', defaults['ruta']))
        self.ent_g1_x.insert(0, cfg.get('g1_x', defaults['g1_x']))
        self.ent_g1_y.insert(0, cfg.get('g1_y', defaults['g1_y']))
        self.ent_g2_x.insert(0, cfg.get('g2_x', defaults['g2_x']))
        self.ent_g2_y.insert(0, cfg.get('g2_y', defaults['g2_y']))
        self.ent_titulo_launcher.insert(0, cfg.get('titulo', defaults['titulo']))
        self.ent_delay_pre_click.insert(0, cfg.get('delay_pre_click', defaults['delay_pre_click']))
        self.ent_delay_launch.insert(0, cfg.get('delay', defaults['delay']))

    def lanzar_game1_game2(self):
        if not UIA_AVAILABLE:
            self.log_status("❌ FALTA LIBRERIA ANTI-BAN. CMD > pip install pywinauto")
            messagebox.showerror("Error Crítico", "Falta 'pywinauto'.\nAbre CMD y escribe:\npip install pywinauto\n\nLuego reinicia el bot.")
            return
        try:
            ...
            delay_pre_click = int(self.ent_delay_pre_click.get())
            delay = int(self.ent_delay_launch.get())
        ...
        except ValueError: self.log_status("❌ Error: Los tiempos deben ser números enteros.")
